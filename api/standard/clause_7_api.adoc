...
|Mode |`/modes/{modeId}` |GET |n/a |`application/json` |<<get_mode_schema>>
...
Modes is a restriction based on the route operation or type of transport desired when computing the route. For instance, "drone", "truck-shortest" or "public-fastest". The API declares the modes that it supports. Each mode can have its own specific schema, allowing clients to define parameters for the mode when creating a new route.
...
[[rec_mode_domain-names]]
[width="90%",cols="2,6a"]
|===
^|*Recommendation {counter:rec-id}* |*/rec/core/domain-names*
^|A |The domain names SHOULD be selected from the following list:

* **Ground Domain**
** **Definition**: The area of the Earth's surface, including its terrain, cities, and infrastructure.
** **Scope**: The domain extends from the surface of the Earth to the high-water mark.
** **Overlaps**: The Ground Domain overlaps with the Maritime Domain in the landward segment of the littorals (coastal areas).
* **Maritime Domain**
** **Definition**: The oceans, seas, bays, rivers, lakes, estuaries and coastal areas of the Earth.
** **Scope**: The Domain includes the surface, subsurface (underwater).
* **Air Domain**
** **Definition**: The atmosphere, beginning at the Earth's surface and extending upwards.
** **Scope**: The Domain extends to the altitude where atmospheric effects become negligible. This is often considered to be the Kármán line (100 km) or the edge of "near space" (around 70,000 feet, or roughly 20 km).
|===
...
[[get_mode_schema]]
===== Fetch the schema of a mode

This operation returns the schema of the mode with id `modeId`.

[[req_mode-get-mode-schema]]
[width="90%",cols="2,6a"]
|===
^|*Requirement {counter:req-id}* |*/req/core/get-mode-schema*
^|A |The server SHALL support the HTTP GET operation at the path `/mode/{modeId}`
for each mode referenced from the Modes resource at `/modes`.
|===

[[req_mode-get-mode-schema-success]]
[width="90%",cols="2,6a"]
|===
^|*Requirement {counter:req-id}* |*/req/core/get-mode-schema-success*
^|A |A successful execution of the operation SHALL be reported as a response with a HTTP status code `200`.
^|B |The content of that response SHALL be an OpenAPI 3.0 compatible schema.
|===

The following is an example of modes supported in an implementation, together with the specific schemas of each mode.
...
[
  {
    "name": "inspection",
    "description": "Generates a plan for a pipeline inspection",
    "title": "Pipeline Inspection"
  },
    "name": "survey",
    "description": "Generates a plan for a bathymetric survey using multi-beam sonar",
    "title": "Survey Plan"
  }
]
----
...
----
{
  ...
  "properties": {
    ...
    "resolution": {
      "description": "Space between beams",
      "maximum": 100,
      "minimum": 0.1,
      "type": "number"
    }
...
----
{
  ...
  "properties": {
    ...
    "range": {
      "default": 400,
      "description": "Desired Sensor Range",
      "maximum": 500,
      "minimum": 100,
      "type": "integer"
    },
...
This requirement enables the API request to trigger a new action based on a sensor input. Each action can have its own schema, allowing the fine tuning of action behaviour through parameter values.
...
[width="90%",cols="2,6a"]
|===
^|*Recommendation {counter:rec-id}* |*/rec/core/action-names*
^|A |The action names SHOULD be selected from the <<action_names_table>>.
|===

[[action_names_table]]
[#tldr,reftext='{table-caption} {counter:table-num}']
.Recommended of action names, definitions and applicability
[cols="18,18,60,18,18",options="header"]
!===
|loiter |Loiter |Maintain position or circle waypoint for surveillance/waiting. |Seconds |UAV/UUV/USV/UGV
|hover |Hover |Stationary hover above/below waypoint. |Seconds |UAV/UUV/USV
|land |Land |Touch down and stop at waypoint. |Seconds |UAV/UGV/USV
|takeoff |Takeoff |Ascend/depart from waypoint surface. |Seconds |UAV/UGV/USV
|dock |Dock |Connect to charging/refueling station. |Seconds |UUV/USV/UGV
|undock |Undock |Disconnect from station. |Seconds |UUV/USV/UGV
|inspect |Inspect |Sensor scan (camera/LiDAR/sonar) of waypoint area. |Seconds |All
|photo |Photo |Capture image/multispectral photo. |Seconds |UAV/UUV/USV
|video |Video |Record video stream. |Seconds |UAV/UUV/USV
|sample |Sample |Collect physical/environmental sample. |Seconds |UAV/UUV/USV/UGV
|drop |Drop |Release payload/object. |Seconds |UAV/USV/UGV
|pick |Pickup |Retrieve payload/object. |Seconds |UAV/USV/UGV
|measure |Measure |Take precise measurement (alt/depth/temp/etc.). |Seconds |All
|relay |Relay |Act as comms/data relay node. |Seconds |All
|sync |Sync |Coordinate timing with other assets. |Seconds |All
|wait |Wait |Pause until condition/time/external trigger. |Seconds |All
|orbit |Orbit |Fly predefined pattern around waypoint. |Seconds |UAV/UUV/USV
|spiral |Spiral |Spiral ascent/descent through waypoint. |Seconds |UAV/UUV
|figure8 |Figure-8 |Fly figure-8 pattern through waypoint. |Seconds |UAV/UUV/USV
|scan |Scan |Sweeping sensor coverage (radar/sonar/LiDAR). |Seconds |All
|map |Map |Generate local 3D map/cloud. |Seconds |UAV/UUV/USV/UGV
|charge |Charge |Perform battery recharge cycle. |Seconds |All
|refuel |Refuel |Perform fuel transfer. |Seconds |USV/UGV
|service |Service |Run self-diagnostic/maintenance routine. |Seconds |All
|report |Report |Transmit status/data summary. |Seconds |All
|abort |Abort |Cancel mission, initiate safe return. |N/A |All
|divert |Divert |Switch to alternate route/mission. |N/A |All
|hold |Hold |Pause all motion, maintain attitude. | Seconds |All 
!===
...
[[get_action_schema]]
===== Fetch the schema of an action

This operation returns the schema of the action with id `actionId`.

[[req_action-get-action-schema]]
[width="90%",cols="2,6a"]
|===
^|*Requirement {counter:req-id}* |*/req/core/get-action-schema*
^|A |The server SHALL support the HTTP GET operation at the path `/action/{actionId}`
for each mode referenced from the Actions resource at `/actions`.
|===

[[req_action-get-action-schema-success]]
[width="90%",cols="2,6a"]
|===
^|*Requirement {counter:req-id}* |*/req/core/get-action-schema-success*
^|A |A successful execution of the operation SHALL be reported as a response with a HTTP status code `200`.
^|B |The content of that response SHALL be an OpenAPI 3.0 compatible schema.
|===

The following is an example of the specific schemas of actions listed in <<example_actions>>.

[[example_action_land]]
.Schema of action land

Response of HTTP GET operation at the path `/actions/land`.
...
----
{
  ...
  "$id": "https://example.com/schemas/land.schema.json",
  "title": "Land",
  "allOf": [
    {
...
[[example_action_land_encoding]]
.Encoding of action land

Encoding of a valid land action
...
[[example_action_loiter]]
.Schema of action loiter
...
[[example_action_loiter_encoding]]
.Encoding of action loiter
