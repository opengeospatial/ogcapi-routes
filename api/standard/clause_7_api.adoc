...
^|B |By default the response content SHALL conform to the requirements class "Route Exchange Model".  NOTE:  This requirements class provides no mechanism to change the default, and future extentions could return another route representation, such as a OGC GeoPackage.
|===
...
This requirement enables the API requeste to specify a vehicle weight in metric tons (tonnes) against known weight restrictions when computing the route.
